...
public class RecaudacionesJOINAlumnoJOINConceptoJOINFacultadDAOImpl implements IRecaudacionesJOINAlumnoJOINConceptoJOINFacultadDAO {
	...
	public List<RecaudacionesJOINAlumnoJOINConceptoJOINFacultad> getRecaudacionesJOINAlumnoJOINConceptoJOINFacultadByNomApe(String nomApe) {
		...
		"r.cod_alumno, r.observacion, " +
		"u.descripcion as descripcion_ubi, t.descripcion as descripcion_tipo " + 
...
/*new*/@Override
	...
	@Override
	public boolean updateRecaudacionesJOINAlumnoJOINConceptoJOINFacultad(Integer id_concepto,String moneda,Date fecha, String numero,int ciclo,int idRec, int importe) {
		
		logger.info("Facultad DAO "+fecha+" "+" "+numero+" "+idRec);
		
		String sql = "UPDATE recaudaciones SET id_concepto=?,moneda=?,fecha = ?, numero = ?,ciclo=?, importe=? WHERE id_rec = ?";
		logger.info("Facultad DAO "+sql);
		Integer resp = jdbcTemplate.update(sql,id_concepto,moneda,fecha,numero,ciclo,importe,idRec);
